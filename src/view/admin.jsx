...
function Admin({ onLogout }) {
  ...
  const resetarSenha = (usernameParaResetar) => {
    if (usernameParaResetar.toLowerCase() === 'admin') {
      alert('Não é possível resetar a senha do administrador principal por aqui.');
      return;
    }

    // Abre uma caixa no navegador para o admin definir a senha temporária
    const novaSenhaTemp = prompt(`Digite uma nova senha temporária para o usuário "${usernameParaResetar}":`);
    ...
    alert(`Senha de "${usernameParaResetar}" resetada com sucesso!\nInforme a nova senha temporária ao usuário.`);
  };
  ...
  const deletarUsuario = (usernameParaDeletar) => {
    ...
    if (confirm(`Tem certeza que deseja apagar permanentemente o usuário "${usernameParaDeletar}" e todas as suas listas?`)) {
      // 1. Remove da lista de logins
      const novosUsuarios = usuarios.filter(u => u.username !== usernameParaDeletar);
      localStorage.setItem('usuarios_registrados', JSON.stringify(novosUsuarios));
      setUsuarios(novosUsuarios);
  ...
  return (
    <div style={styles.container}>
      <header style={styles.header}>
        ...
        <button onClick={onLogout} style={styles.btnSair}>Sair do Painel</button>
      </header>

      {/* --- SEÇÃO 1: GESTÃO DE USUÁRIOS --- */}
      <section style={styles.section}>
        <h3 style={styles.subtitulo}>👥 Gestão de Usuários Cadastrados ({usuarios.length})</h3>
      ...
      <section style={styles.section}>
        ...
        <div style={styles.logsContainer}>
          {logsFiltrados.length === 0 ? (
            <p style={{ color: '#94a3b8', textAlign: 'center' }}>Nenhuma atividade registrada hoje.</p>
          ) : (
              ...
              {logsFiltrados.map((log) => (
                <div key={log.id} style={styles.logCard}>
                  ...
                  <p style={styles.logTexto}>
                    O usuário <strong style={{ color: '#3b82f6' }}>{log.usuario}</strong>{' '}
                    <span style={styles.acaoBadge(log.acao)}>{log.acao}</span> a tarefa:{' '}
                    <em style={{ color: '#f8fafc' }}>"{log.tarefa}"</em>
                  </p>
                </div>
              ))}
            </div>
...
const styles = {
  container: { maxWidth: '900px', margin: '40px auto', padding: '20px', color: '#f8fafc', fontFamily: 'sans-serif' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #334155', paddingBottom: '20px', marginBottom: '30px' },
  subtitulo: { color: '#3b82f6', marginBottom: '15px' },
  section: { backgroundColor: '#1e293b', padding: '25px', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.2)', marginBottom: '30px' },
  ...
  senhaMascarada: { color: '#64748b', letterSpacing: '3px', fontSize: '18px', fontFamily: 'monospace' },
  inputBusca: { width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #334155', backgroundColor: '#0f172a', color: '#fff' },
  logsContainer: { maxHeight: '300px', overflowY: 'auto', paddingRight: '5px' },
  logCard: { backgroundColor: '#0f172a', padding: '12px', borderRadius: '8px', borderLeft: '4px solid #3b82f6' },
  logData: { fontSize: '11px', color: '#64748b', display: 'block', marginBottom: '4px' },
  logTexto: { fontSize: '14px', color: '#cbd5e1' },
  btnSair: { padding: '8px 16px', background: '#ef4444', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' },
  btnDeletar: { padding: '6px 12px', background: 'none', border: '1px solid #ef4444', color: '#ef4444', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' },
  btnResetar: { padding: '6px 12px', background: 'none', border: '1px solid #3b82f6', color: '#3b82f6', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' },
  acaoBadge: (acao) => {
    let cor = '#10b981';
    if (acao === 'Adicionou') cor = '#3b82f6';
    if (acao === 'Removeu') cor = '#f59e0b';
    return { backgroundColor: `${cor}22`, color: cor, padding: '2px 6px', borderRadius: '4px', fontWeight: 'bold', fontSize: '12px' };
  }
};
